...
function normalizeAsciiSlugForLookup(value: string): string {
  return normalizeSlugForLookup(value).normalize('NFKD').replace(/\p{Mark}+/gu, '')
}
...
function loadPosts(): BlogPost[] {
  ...
  const withSlugs = loaded.map(post => ({
    ...post,
    slug: (collisionCounts.get(`${post.locale}:${post.slugCandidate}`) ?? 0) > 1
...
export function getPost(slug: string, locale: SupportedLocale): BlogPost | null {
  ...
  const post = localePosts.find(entry => entry.slug === slug || entry.slug === decoded)
    ?? localePosts.find(entry => entry.contentSlug === slug || entry.contentSlug === decoded)
    ?? localePosts.find(entry => normalizeSlugForLookup(entry.slug) === normalized
      || normalizeSlugForLookup(entry.contentSlug) === normalized)
    ?? localePosts.find(entry => normalizeAsciiSlugForLookup(entry.slug) === ascii
...
export function listPostSitemapEntries(): BlogPostSitemapEntry[] {
  ...
  return [...grouped].map(([contentSlug, posts]) => ({
    contentSlug,
    locales: posts.map(post => post.locale).toSorted(),
    localizedSlugs: Object.fromEntries(posts.map(post => [post.locale, post.slug])),
    lastModified: posts.map(post => post.lastModified).filter((date): date is Date => Boolean(date))
