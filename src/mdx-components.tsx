  ...
  return {
    a: ({ href, children, ...rest }) => {
      ...
      if (safeHref.startsWith('/') && !isStaticOrApi) {
        return <Link href={safeHref} locale={locale} {...rest}>{children}</Link>
      }
    ...
    img: ({ src, alt, width, height }) => {
      ...
      return (
        <figure>
          <Image src={resolvedSrc} alt={alt ?? ''} width={Number(width) || 1600} height={Number(height) || 900} sizes="(max-width: 768px) 100vw, 800px" />
          {alt ? <figcaption>{alt}</figcaption> : null}
        </figure>
      )
    },
    table: ({ children }) => <div className="blog-prose-table-wrap"><table>{children}</table></div>,
    Callout,
    Stat,
    MarketEmbed,
    Comparison,
