...
export function rawToCentsWithRemainder(params: { amountRaw: string; decimals: number; remainderRaw?: string }) {
  ...
  const total = BigInt(integerString(params.amountRaw)) + BigInt(integerString(params.remainderRaw || '0'))
...
export function addRawAmounts(values: Iterable<string>) {
  ...
  for (const value of values) {
    total += BigInt(integerString(value))
  }
