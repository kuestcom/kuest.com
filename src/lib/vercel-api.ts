...
function extractRequiredMetadataKeys(error: LaunchError) {
  ...
  for (const match of message.matchAll(/metadata\.([a-zA-Z0-9_]+)\s*:\s*required/gi)) {
    if (match[1]) {
      keys.add(match[1]);
    }
  }

  const fields = (error.details as { error?: { fields?: unknown[] } } | undefined)?.error
    ?.fields;
  if (Array.isArray(fields)) {
    for (const field of fields) {
      ...
      for (const match of fieldMessage.matchAll(/metadata\.([a-zA-Z0-9_]+)/gi)) {
        if (match[1]) {
          keys.add(match[1]);
        }
      }
...
export async function connectSupabaseViaVercelIntegration(params: {
  ...
  const tryAttachReusableResource = async (
