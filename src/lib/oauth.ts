...
export async function setTemporaryOAuthState(cookieName: string, state: OAuthTempState) {
  const store = getRequestCookies()
...
export async function ensureOAuthStateValid(params: { cookieName: string; state?: string | null }) {
