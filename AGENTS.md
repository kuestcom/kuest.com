...
- Stack: Next.js App Router, TypeScript, Tailwind, Playwright, Supabase. Main code lives in `src/*`; static and data assets live in `public` and `docs`.
- Node: use Node.js 24.x from `.nvmrc`.
- Commands: Run `npm run build` and `npm run lint` only when the user explicitly asks.
